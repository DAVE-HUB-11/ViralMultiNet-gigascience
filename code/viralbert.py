...
class MultiScaleDNADataset(Dataset):
    def __init__(self, df, kmer_scales=None, anno_embs=None):
        ...
        assert all(0 <= l < CONFIG["num_classes"] for l in
                   self.labels.tolist()), f"Label out of range: {set(self.labels.tolist())}"
...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, query_modal, key_value_modal, attention_mask=None):
        ...
        k = self.k_proj(key_value_modal).reshape(B, L_kv, self.num_heads, self.head_dim).transpose(1,
                                                                                                   2)  # [B, H, L_kv, D]
        v = self.v_proj(key_value_modal).reshape(B, L_kv, self.num_heads, self.head_dim).transpose(1,
                                                                                                   2)  # [B, H, L_kv, D]
...
class MultiScaleModel(nn.Module):
    def __init__(self, kmer_scales, num_layers, hidden_size, out_dim, num_attention_heads, class_weights=None,
                 focal_loss=None, anno_dim=768, tokenizers=None, is_teacher=False):
        super().__init__()
...
class DistillTrainer:
    def __init__(self, stage="teacher", train_df=None, val_df=None):
        ...
        if CONFIG.get("enable_prompt_embed", False):
            try:
                ...
                self.prompter = PromptEmbedder(
                    model_name=CONFIG.get("prompt_model_name", "sentence-transformers/all-mpnet-base-v2"))
            except Exception as e:
        ...
        if stage == "teacher":
            ...
            if os.path.exists(pretrained_path):
                ...
                state_dict = torch.load(pretrained_path, map_location=DEVICE,
                                        weights_only=True)  # Increase weights_only=True to avoid warning
                        ...
                        for i, scale in enumerate(CONFIG["kmer_scales"]):
                            ...
                            for j, s in enumerate(CONFIG["kmer_scales"]):
                                if j == i:
                                    ...
                                    embeddings_dict[f'embeddings_{s}'] = embedding_layer(
                                        batch[f'input_ids_{s}']).detach()
        ...
        for epoch in range(CONFIG["num_epochs"]):
            ...
            for batch in pbar:
                ...
                if self.rag is not None and CONFIG.get("enable_rag", False):
                    try:
                        ...
                        if query_texts:
                            ...
                            batch['retrieved_emb'] = torch.tensor(retrieved_embs, dtype=torch.float,
                                                                  device=DEVICE).mean(dim=1)
                    except Exception as e:
                        print(f"[RAG] Batch retrieval skipped due to error: {e}")
                # Prompt-based embedding per batch (optional)
                if self.prompter is not None and CONFIG.get("enable_prompt_embed", False) and CONFIG.get(
                ...
                for i, (s_feat, t_feat) in enumerate(
                        zip(student_outputs.get("features", []), teacher_outputs.get("features", []))):
                ...
                if student_attentions and teacher_attentions:
                    for s_attn, t_attn in zip(student_attentions, teacher_attentions):
                        attn_loss += self.calculate_distillation_loss(s_attn[-1],
                                                                      t_attn[-1])  # Use last layer attention
    ...
    def evaluate(self, epoch, silent=False):
        ...
        try:
            auc = roc_auc_score(label_binarize(all_labels, classes=np.arange(CONFIG["num_classes"])), all_probs,
                                multi_class='ovr', average='macro')
        except Exception:
            auc = 0.5
        try:
            auc_pr = average_precision_score(label_binarize(all_labels, classes=np.arange(CONFIG["num_classes"])),
        ...
        minority_recall = recall_score(all_labels, all_preds, average=None)[
            minority_class_index]  # Assume minority_class_index known
    ...
    def _make_query_texts(self, batch):
        try:
            ...
            if hasattr(self, 'train_dataset') and hasattr(self.train_dataset, 'tokenizers') and len(
                    self.train_dataset.tokenizers) > 0:
                tok = self.train_dataset.tokenizers[0]
                input_ids_1 = batch.get('input_ids_4') or next((batch[k] for k in batch if k.startswith('input_ids_')),
                                                               None)
                ...
                for ids in ids_cpu:
                    ...
                    tok.pad_token, tok.cls_token if hasattr(tok, 'cls_token') else None,
                    tok.sep_token if hasattr(tok, 'sep_token') else None)]
...
def objective(trial):
    ...
    if not os.path.exists(CONFIG["teacher_path"]):
        raise FileNotFoundError(
            f"Teacher model not found at {CONFIG['teacher_path']}. Please train the teacher model first.")
    ...
    trainer.train_loader = DataLoader(train_dataset, batch_size=CONFIG["batch_size"], shuffle=True,
                                      num_workers=CONFIG["num_workers"])
    trainer.val_loader = DataLoader(val_dataset, batch_size=CONFIG["batch_size"], shuffle=False,
                                    num_workers=CONFIG["num_workers"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rag_corpus_path", type=str, default=None,
                        help="Text corpus file for RAG index (one doc per line)")
    ...
    if args.stage in ["student", "student_nodistill"]:
        if args.enable_vllm:
            ...
            save_for_vllm(trainer.model, "vllm_student_model")
